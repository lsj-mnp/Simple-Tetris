...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	while (g_simpleTetris.update() == true)
	{
		...
		if (g_simpleTetris.isGameStart() == true)
		{
			//gameover가 아닌 상황.
				...
				if (g_simpleTetris.tickInput() == true)
				{
					...
					else
					{
						...
						if (GetAsyncKeyState(VK_UP) == SHORT(0x8001))
						{
							...
							g_simpleTetris.playFastDownSound();
		...
		if (GetAsyncKeyState('S') == SHORT(0x8001))
		{
			g_simpleTetris.gameStart();
		}
		...
		g_simpleTetris.beginRendering(clearColor);
		{
			...
			else
			{
				...
				g_simpleTetris.drawTextToScreen(Position2(0, 0), Size2(g_kWidth, g_kHeight), L"press 'S' key"
			...
			if (g_simpleTetris.isGameOver() == true)
			{
				...
				g_simpleTetris.drawTextToScreen(Position2(0, 0), Size2(g_kWidth, g_kHeight), L"GAME OVER"
					, normalFontColor - Color(255, 255, 255), EHorzAlign::Center, EVertAlign::Center);
